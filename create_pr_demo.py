...
""",
    ...
    # Step 10: Switch back
    print("\nStep 10: Switching back to dev...")
    run_git_command(["git", "checkout", "dev"], repo_path)
    print("✅ Done!")
...
def main():
    ...
    # Step 2: Checkout to dev branch to get latest
    print(f"\nStep 2: Switching to {base_branch} branch...")
    success, output = run_git_command(["git", "checkout", base_branch], repo_path)
    if not success:
        # Try origin/dev if local dev doesn't exist
        success, output = run_git_command(["git", "checkout", "-b", base_branch, f"origin/{base_branch}"], repo_path)
        if not success:
            print(f"❌ Failed to checkout {base_branch}: {output}")
            return
    ...
    # Step 4: Create and checkout new feature branch
...
"""
    ...
    print("\nStep 8: Pushing branch to remote...")
    success, output = run_git_command(
        ["git", "push", "-u", "origin", branch_name],
        repo_path
    )
    ...
    # Step 9: Create Pull Request using our client
    print("\nStep 9: Creating Pull Request...")
    print("-" * 40)
