...
def upsert_data(conn, schema_name: str, target_table: str, metadata: Dict[str, Any]) -> None:
    ...
    Uses proper PostgreSQL syntax with table name qualification to avoid ambiguity.
    In ON CONFLICT DO UPDATE WHERE clause, use table_name.column to reference existing row.
    ...
    try:
        ...
        # - Use table_name.column to reference existing row (PostgreSQL requires this to avoid ambiguity)
        # - EXCLUDED.column refers to new values
        # - This prevents updates when data hasn't changed
        # Use just table name (not schema.table) as PostgreSQL resolves it from INSERT context
        table_alias = target_table
        
        where_conditions = [
            f"{table_alias}.glue_catalog_table_type IS DISTINCT FROM EXCLUDED.glue_catalog_table_type",
            f"{table_alias}.glue_catalog_table_storage_location IS DISTINCT FROM EXCLUDED.glue_catalog_table_storage_location",
            f"{table_alias}.glue_catalog_table_input_format IS DISTINCT FROM EXCLUDED.glue_catalog_table_input_format",
            f"{table_alias}.glue_catalog_table_output_format IS DISTINCT FROM EXCLUDED.glue_catalog_table_output_format",
            f"{table_alias}.glue_catalog_table_serde_library IS DISTINCT FROM EXCLUDED.glue_catalog_table_serde_library",
            f"COALESCE({table_alias}.glue_catalog_table_serde_parameters::jsonb, '{{}}'::jsonb) IS DISTINCT FROM COALESCE(EXCLUDED.glue_catalog_table_serde_parameters::jsonb, '{{}}'::jsonb)",
            f"COALESCE({table_alias}.glue_catalog_table_columns_metadata::jsonb, '[]'::jsonb) IS DISTINCT FROM COALESCE(EXCLUDED.glue_catalog_table_columns_metadata::jsonb, '[]'::jsonb)",
            f"COALESCE({table_alias}.glue_catalog_table_partition_keys_metadata::jsonb, '[]'::jsonb) IS DISTINCT FROM COALESCE(EXCLUDED.glue_catalog_table_partition_keys_metadata::jsonb, '[]'::jsonb)",
            f"{table_alias}.glue_catalog_table_connection_name IS DISTINCT FROM EXCLUDED.glue_catalog_table_connection_name",
            f"COALESCE({table_alias}.glue_catalog_table_parameters::jsonb, '{{}}'::jsonb) IS DISTINCT FROM COALESCE(EXCLUDED.glue_catalog_table_parameters::jsonb, '{{}}'::jsonb)",
            f"{table_alias}.glue_dq_ruleset_description IS DISTINCT FROM EXCLUDED.glue_dq_ruleset_description",
            f"{table_alias}.glue_dq_ruleset_dqdl_rules IS DISTINCT FROM EXCLUDED.glue_dq_ruleset_dqdl_rules",
            f"{table_alias}.glue_dq_ruleset_created_timestamp IS DISTINCT FROM EXCLUDED.glue_dq_ruleset_created_timestamp",
            f"{table_alias}.glue_dq_ruleset_last_modified_timestamp IS DISTINCT FROM EXCLUDED.glue_dq_ruleset_last_modified_timestamp"
        ]
